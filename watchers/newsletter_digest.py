...
def build():
    ...
    if pub_items or gated_items:
        ...
        for it in pub_items:
            dept_rows += ("<div style='border-left:3px solid #1f6f54;padding:.5rem .8rem;margin:.6rem 0'>"
                          ...
                          ("<a href='%s' style='font-size:.8rem;color:#0e4a84'>Source &#8599;</a>" % esc(it["source_href"])
        ...
        for it in gated_items:
            ...
            dept_lines.append("  [public question] %s\n    %s%s" % (it["headline"], it["angle"],
                               ("\n    Source: %s" % it["source_href"]) if it.get("source_href") else ""))
        dept_html = ("<h2 style='color:#0e4a84;font-size:1.05rem;margin-top:1.1rem'>"
    ...
    html_issue = (
        ...
        "<body style='margin:0;background:#f4f7fb;font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:#1f2d3a'>"
        ...
        "<h2 style='color:#0e4a84;font-size:1.05rem'>By the numbers</h2>"
        ...
        "%s"
        "<h2 style='color:#0e4a84;font-size:1.05rem;margin-top:1.1rem'>Dig in yourself</h2>"
